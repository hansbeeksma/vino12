...
import { colorHex, bodyToNumber } from "@/lib/utils";
...
export function WineJourney() {
  ...
  return (
    <section id="reis" className="section-padding bg-champagne border-y-brutal border-ink">
      <div className="container-brutal">
        ...
        <p className="font-body text-xl text-ink/70 max-w-lg mb-8">
          ...
          Cabernet Sauvignon. Van frisse Sauvignon Blanc tot weelderige Viognier.
        </p>
      </div>

      {/* Horizontaal scrollende journey */}
      <div
        ref={scrollRef}
            ...
            <div className="flex items-center gap-0">
              {reds.map((wine, i) => (
                <div key={wine.id} className="flex items-center">
                  ...
                  >
                    {/* Node */}
                    <div className="w-12 h-12 border-brutal border-ink bg-offwhite brutal-shadow brutal-hover flex items-center justify-center relative">
                      <div
                        className="w-6 h-6 border border-ink"
                        style={{
                          backgroundColor: colorHex(wine.color),
                          opacity: 0.3 + bodyToNumber(wine.body) * 0.14,
                        }}
                      />
                      <span className="absolute -top-6 font-accent text-[10px] text-ink/40">
                        #{wine.id}
                      </span>
                    </div>
                    {/* Label */}
